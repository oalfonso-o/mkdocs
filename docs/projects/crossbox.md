...
##### Fee
These are the plans that the users can select, they have an price and an amout of WODs to get for that price. The payment is monthly, processed with Stripe.
...
##### Session
Users reservate in sessions, which have an specific day and specific time.
[Code](https://github.com/oalfonso-o/crossbox/blob/master/crossbox/models/session.py)

##### SessionTemplate
It's the same as a session but without relation to a real day, it's only related to a WeekTemplate, it's day is only Monday, Tuesday, etc. It's used to recreate sessions across upcoming weeks.
...
##### Payment
Every Stripe charge generates a Payment, is just an historic.
[Code](https://github.com/oalfonso-o/crossbox/blob/master/crossbox/models/payment.py)

### User Usage
The idea was to keep the usage for the users as [simple](https://en.wikipedia.org/wiki/KISS_principle) as possible. So users register with a simple form, they add their peyment method, they search for their desired day/hour and they click a simple checkbox to book their training session.
